...
> Para utilizar _moveVis_ es recomendable que utilicen su computadora personal ya sea R Studio o Positron, debido a que este paquete demanda mucha memoria y procesamiento que Posit Cloud no puede proporcionar.
...
```{r eval = FALSE}
library(pacman)
p_load(move, slippymath, magick, gifski, av, pbapply)
```
...
```{r eval = FALSE}
# Crear el objeto que almacene los frames de la animacion
...
```{r eval = FALSE}
frames_edit <- add_labels(frames, x = "Longitud" , y = "Latitud", title = "Movimientos del jaguar Courisco en Brasil") %>%
  add_scalebar(height = 0.015) %>%
  add_northarrow() %>%
  add_timestamps(courisco_move, type = "label")

# Explora uno de los frames editados
frames_edit[[535]]
```
